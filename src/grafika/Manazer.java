 ...
 * 
 * Triedu Manazer som celú zobral z projektu RPSLS na moodli
 */
    ...
    private class ManazerMysi extends MouseAdapter {
        public void mouseClicked(MouseEvent event) {
            if (event.getButton() == MouseEvent.BUTTON1) {
                Manazer.this.posliSpravu("lavyKlik", event.getX(), event.getY());
            }
            
            if (event.getButton() == MouseEvent.BUTTON3) {
                Manazer.this.posliSpravu("pravyKlik", event.getX(), event.getY());
            }
        }
    }
